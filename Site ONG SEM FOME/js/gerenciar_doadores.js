...
function validaCPF(v) {
  const d = onlyDigits(v);
  if (d.length !== 11 || isSeq(d)) return false;
  let s = 0;
...
function validaCNPJ(v) {
  const d = onlyDigits(v);
  if (d.length !== 14 || isSeq(d)) return false;
  const calc = (len) => {
    const w =
      len === 12
    ...
    let s = 0;
    for (let i = 0; i < w.length; i++) s += +d[i] * w[i];
    const r = s % 11;
    return r < 2 ? 0 : 11 - r;
  };
  const dv1 = calc(12);
...
async function createDoador(payload) {
  // POST /api/doadores
  const r = await fetch("/api/doadores", {
    method: "POST",
    headers: {
  ...
  if (!r.ok) throw new Error("Falha ao criar doador");
  return r.json();
}

async function updateDoador(id, payload) {
  // PUT /api/doadores/:id
  const r = await fetch(`/api/doadores/${id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!r.ok) throw new Error("Falha ao atualizar doador");
  return r.json();
}

async function deleteDoadorAPI(id) {
  // DELETE /api/doadores/:id
  const r = await fetch(`/api/doadores/${id}`, { method: "DELETE" });
  if (!r.ok) throw new Error("Falha ao excluir doador");
}
...
// Animação do modal (fade-in/fade-out)
...
function abrirModalDoador(editar = false, doador = {}) {
  ...
  document.getElementById("tituloModalDoador").textContent = editar
    ? "Editar Doador"
    : "Adicionar Doador";
...
// Fecha o modal
function fecharModalDoador() {
  hideModalWithAnim(document.getElementById("modalDoador"));
}

// Ações de abrir/fechar modal
document.getElementById("btnAdicionarDoador").onclick = () =>
  abrirModalDoador();
...
document.getElementById("formDoador").onsubmit = function (e) {
  ...
  document
    .getElementById("nomeDoador")
    .setCustomValidity(nomeOk ? "" : "Informe o nome");
  document
    .getElementById("emailDoador")
    .setCustomValidity(emailOk ? "" : "E-mail inválido");
  document
    .getElementById("telefoneDoador")
    .setCustomValidity(telOk ? "" : "Telefone inválido");
  document
    .getElementById("documentoDoador")
    .setCustomValidity(docOk ? "" : "CPF/CNPJ inválido");

  if (!nomeOk || !emailOk || !telOk || !docOk) {
    document.getElementById("formDoador").reportValidity();
    return;
  }
  ...
  (async () => {
    try {
      ...
      if (id) await updateDoador(id, payload);
      else await createDoador(payload);
...
if (docInput) {
  docInput.addEventListener("input", (e) => {
    ...
    e.target.setCustomValidity(
      validaDoc(e.target.value) ? "" : "CPF/CNPJ inválido"
    );
  });
