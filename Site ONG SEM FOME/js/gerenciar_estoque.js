...
const normalizeCategoria = (v) => (v || "").trim().toLowerCase();
const normalizeUnidade = (v) => (v || "").trim().toLowerCase();

function saldoSemEntrada(idExcluir, categoria, unidade) {
  const catNorm = normalizeCategoria(categoria);
  const unNorm = normalizeUnidade(unidade);
  return entradas.reduce((total, ent) => {
    if (idExcluir && ent.idEntrada === idExcluir) return total;
    if (
      normalizeCategoria(ent.categoria) === catNorm &&
      normalizeUnidade(ent.unidade) === unNorm
    ) {
      return total + (Number(ent.quantidade) || 0);
    }
    return total;
  }, 0);
}

function isDistribuicaoCestasEntrada(entry) {
  if (!entry) return false;
  const cat = normalizeCategoria(entry.categoria);
  const doador = (entry.doador || "").trim().toUpperCase();
  const obs = entry.obs || "";
  return (
    cat === "cesta básica" && (doador === "SAIDA" || obs.startsWith("Saída #"))
  );
}

function isMontagemEntrada(entry) {
  if (!entry) return false;
  const doador = (entry.doador || "").trim().toUpperCase();
  const obs = (entry.obs || "").toLowerCase();
  return doador === "MONTAGEM" || obs.includes("montagem #");
}

function isSaidaManualEntrada(entry) {
  if (!entry) return false;
  if (isDistribuicaoCestasEntrada(entry) || isMontagemEntrada(entry))
    return false;
  return (
    (entry.tipo || "").toLowerCase() === "saida" ||
    Number(entry.quantidade || 0) < 0
  );
}
...
function buildEstoque(list) {
  ...
  estoque = Array.from(map.values()).sort(
    (a, b) =>
...
function renderMovimentacoes() {
  ...
  const und = (document.getElementById("movFiltroUnidade")?.value || "")
    .trim()
    .toLowerCase();
  const camp = (
    document.getElementById("movFiltroCampanha")?.value || ""
  ).trim();
  ...
  const q = (document.getElementById("filtroBusca")?.value || "")
    .trim()
    .toLowerCase();
  ...
  if (!pageItems.length) {
    ...
    tr.innerHTML =
      '<td colspan="9" style="text-align:center;color:#666;">Nenhuma movimentação</td>';
    tbody.appendChild(tr);
    return;
  }
  pageItems.forEach((m) => {
    const tr = document.createElement("tr");
    const categoriaLower = normalizeCategoria(m.categoria);
    const isDistribuicaoCestas =
      m.tipo === "Saída" &&
      categoriaLower === "cesta básica" &&
      ((m.doador || "").trim().toUpperCase() === "SAIDA" ||
        (m.obs || "").startsWith("Saída #"));
    const isMontagem =
      m.doador === "MONTAGEM" || (m.obs || "").includes("montagem #");
    const isSaidaManual =
      !isDistribuicaoCestas &&
      !isMontagem &&
      String(m.entrada_tipo || "").toLowerCase() === "saida";
    let origemDescricao = "";
    if (isDistribuicaoCestas) {
      origemDescricao = "Distribuição de cestas";
    } else if (m.tipo === "Entrada") {
      const entradaTipo = String(m.entrada_tipo || "").toLowerCase();
      const origemBase =
        entradaTipo === "compra" ? "Entrada (compra)" : "Entrada (doação)";
      const extra =
        entradaTipo === "compra"
          ? m.fornecedor
            ? ` - ${m.fornecedor}`
          ...
          ? ` - ${m.doador}`
          : "";
      origemDescricao = `${origemBase}${extra}`;
    } else if (isSaidaManual) {
      const dest = m.doador && m.doador !== "-" ? ` - ${m.doador}` : "";
      origemDescricao = `Saída de produtos${dest}`;
    } else if (isMontagem) {
      origemDescricao = "Montagem de cestas";
    } else {
      origemDescricao = m.doador || "-";
    }
    const temObs = m.obs && m.obs !== "-";
    let obsDisplay = temObs ? m.obs : "";
    if (
      m.tipo === "Entrada" &&
      String(m.entrada_tipo || "").toLowerCase() === "compra" &&
      m.forma_pagamento
    ) {
      const detalhePagamento = `Pagamento: ${m.forma_pagamento}`;
      obsDisplay = obsDisplay
        ? `${obsDisplay} | ${detalhePagamento}`
        : detalhePagamento;
    }
    let acoes = "";
    if (isMontagem) {
      acoes = "";
    } else if (isDistribuicaoCestas) {
      acoes = `
    ...
    const acoesHtml = acoes || '<span class="text-muted">—</span>';
    tr.innerHTML = `
      <td>${formatDateTime(m.data, m.criado_em)}</td>
      <td>${m.tipo}</td>
      <td>${m.categoria}</td>
      <td>${m.unidade}</td>
      <td style="color:${m.quantidade < 0 ? "#b00020" : "#0a7"};">${
      m.quantidade
    }</td>
      <td>${origemDescricao}</td>
      <td>${m.campanha}</td>
      <td>${obsDisplay || "-"}</td>
      <td>${m.saldo}</td>
      <td>${acoesHtml}</td>
    ...
    tr.querySelectorAll("button[data-acao]").forEach((btn) => {
      ...
      if (acao === "editar-entrada")
        btn.addEventListener("click", () => editarEntrada(id));
      if (acao === "excluir-entrada")
        btn.addEventListener("click", () => excluirEntrada(id));
      if (acao === "editar-saida")
        btn.addEventListener("click", () => editarSaidaByEntradaId(id));
      if (acao === "excluir-saida")
        btn.addEventListener("click", () => excluirSaidaByEntradaId(id));
    });
    tbody.appendChild(tr);
  });
}
...
function renderTabelaEstoque() {
  ...
  totalEstoquePages = Math.max(
    1,
    Math.ceil(estoqueFiltrado.length / limitEstoque)
  );
  ...
  if (!pageItems.length) {
    const tr = document.createElement("tr");
    tr.innerHTML =
      '<td colspan="5" style="text-align:center;color:#666;">Nenhum item encontrado</td>';
    tbody.appendChild(tr);
    return;
  }
  ...
  pageItems.forEach((i, localIdx) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-edit" data-idx="${
          start + localIdx
        }">Detalhes</button>
      </td>
    `;
    tr.querySelector("button").addEventListener("click", () =>
      abrirDetalhes(start + localIdx)
    );
    tbody.appendChild(tr);
  });
}
...
function renderDetalhesFiltrados() {
  ...
  const busca = (document.getElementById("detBusca")?.value || "")
    .toLowerCase()
    .trim();
  ...
  if (info)
    info.textContent = `Página ${pageDetalhes} de ${totalDetalhesPages}`;
  ...
  if (!pageItems.length) {
    const tr = document.createElement("tr");
    tr.innerHTML =
      '<td colspan="6" style="text-align:center;color:#666;">Sem resultados</td>';
    tbody.appendChild(tr);
    return;
  }
  pageItems.forEach((e) => {
...
if (movCat)
  movCat.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movUnd)
  movUnd.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movCamp)
  movCamp.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movTipo)
...
if (movDe)
  movDe.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movAte)
  movAte.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movOrder)
  movOrder.addEventListener("change", () => {
    pageMov = 1;
    renderMovimentacoes();
  });
if (movClear)
  movClear.addEventListener("click", () => {
...
if (detDe)
  detDe.addEventListener("change", () => {
    pageDetalhes = 1;
    renderDetalhesFiltrados();
  });
if (detAte)
  detAte.addEventListener("change", () => {
    pageDetalhes = 1;
    renderDetalhesFiltrados();
  });
if (detDoador)
  detDoador.addEventListener("change", () => {
    pageDetalhes = 1;
    renderDetalhesFiltrados();
  });
if (detCampanha)
  detCampanha.addEventListener("change", () => {
    pageDetalhes = 1;
    renderDetalhesFiltrados();
  });
if (detTipo)
...
if (detBusca)
  detBusca.addEventListener("input", () => {
    pageDetalhes = 1;
    renderDetalhesFiltrados();
  });
if (detClear)
  detClear.addEventListener("click", () => {
...
if (prevEstoqueBtn)
  prevEstoqueBtn.addEventListener("click", () => {
    if (pageEstoque > 1) {
      pageEstoque--;
      renderTabelaEstoque();
    }
...
if (nextEstoqueBtn)
  nextEstoqueBtn.addEventListener("click", () => {
    if (pageEstoque < totalEstoquePages) {
      pageEstoque++;
      renderTabelaEstoque();
    }
  });
if (prevDetalhesBtn)
  prevDetalhesBtn.addEventListener("click", () => {
    if (pageDetalhes > 1) {
      pageDetalhes--;
      renderDetalhesFiltrados();
    }
...
if (nextDetalhesBtn)
  nextDetalhesBtn.addEventListener("click", () => {
    if (pageDetalhes < totalDetalhesPages) {
      pageDetalhes++;
      renderDetalhesFiltrados();
    }
  });
if (prevMovBtn)
  prevMovBtn.addEventListener("click", () => {
    if (pageMov > 1) {
      pageMov--;
      renderMovimentacoes();
    }
...
if (nextMovBtn)
  nextMovBtn.addEventListener("click", () => {
    if (pageMov < totalMovPages) {
      pageMov++;
      renderMovimentacoes();
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (fecharModalEditarMovBtn)
    fecharModalEditarMovBtn.addEventListener("click", fecharEditarMovModal);
  if (cancelarEditarMovBtn)
    cancelarEditarMovBtn.addEventListener("click", fecharEditarMovModal);
  if (formEditarMov)
    formEditarMov.addEventListener("submit", onSubmitEditarMov);
});
...
function openEditarMovModal(data) {
  ...
  const isSaidaManualEntrada =
    isEntrada &&
    ((data.entradaTipo || "").toLowerCase() === "saida" ||
      Number(data.quantidade || 0) < 0);
  if (titulo)
    titulo.textContent = isEntrada ? "Editar Entrada" : "Editar Saída";
  // Campos
  ...
  (async () => {
    try {
      ...
      if (sel) {
        await editCategoriasSelect(
          sel,
          data.categoria || (isSaida ? "Cesta Básica" : "")
        );
  ...
  const qtdInput = document.getElementById("editQuantidade");
  if (qtdInput) {
    if (isEntrada) {
      if (isSaidaManualEntrada) {
        qtdInput.removeAttribute("min");
        qtdInput.title = "Saídas manuais devem permanecer negativas.";
      } else {
        qtdInput.min = "1";
        qtdInput.title = "Entradas devem ser positivas.";
      }
    } else {
      qtdInput.min = "1";
      qtdInput.title = "";
  ...
  (function () {
    ...
    if (!sel)
      return setValue("editUnidade", data.unidade || (isSaida ? "cx" : ""));
    const target = String(
      data.unidade || (isSaida ? "cx" : "") || ""
    ).toLowerCase();
    if (!target) {
      sel.value = "";
      return;
  ...
  preencherEditCampanhaSelect(data.campanha_id ?? "", data.campanha || "");
...
async function onSubmitEditarMov(e) {
  ...
  if (modo === "entrada") {
    ...
    const campanhaRaw = document.getElementById("editCampanha")?.value || "";
    const campanhaNum = Number(campanhaRaw);
    const campanha_id =
      campanhaRaw && !Number.isNaN(campanhaNum) ? campanhaNum : null;
    let categoriaNome = "";
    ...
    const ehSaidaManual = isSaidaManualEntrada(original);
    if (ehSaidaManual) {
      if (!(quantidade < 0)) {
        return alert(
          "Saídas de produtos devem permanecer com quantidade negativa."
        );
      }
      const saldoDisponivel = saldoSemEntrada(id, categoriaNome, unidade);
      const saldoProjetado = saldoDisponivel + quantidade;
      if (saldoProjetado < 0) {
        return alert(
          `Quantidade informada ultrapassa o saldo atual (${saldoDisponivel} ${unidade}).`
        );
      }
    } else {
      if (!(quantidade > 0)) {
        return alert("Entradas devem possuir quantidade positiva.");
      }
    }
  ...
  try {
    ...
    if (currentCategoriaNome) {
      ...
      for (const o of selectEl.options) {
        if (
          String(o.dataset?.nome || "").toLowerCase() ===
          String(currentCategoriaNome).toLowerCase()
        ) {
          matched = true;
...
async function preencherEditCampanhaSelect(selectedValue = "", selectedLabel = "") {
  const sel = document.getElementById("editCampanha");
  if (!sel) return;
  sel.disabled = true;
  try {
    if (!Array.isArray(preencherEditCampanhaSelect.cache)) {
      const r = await fetch("/api/campanhas?limit=1000");
      if (!r.ok) throw new Error("Falha ao carregar campanhas");
      const payload = await r.json();
      preencherEditCampanhaSelect.cache = Array.isArray(payload)
        ? payload
        : payload.data || [];
    }
    const campanhas = preencherEditCampanhaSelect.cache || [];
    ...
    campanhas
      .slice()
      .sort((a, b) => {
        const nomeA = String(a.nome || a.titulo || "").toLowerCase();
        const nomeB = String(b.nome || b.titulo || "").toLowerCase();
    ...
    if (!targetValue && selectedLabel) {
      const matchCamp = campanhas.find((camp) => {
        const nome = (camp.nome || camp.titulo || '').toLowerCase();
        return nome && nome === selectedLabel.toLowerCase();
      });
      if (matchCamp) {
        const id = matchCamp.id_campanha ?? matchCamp.id;
        if (id != null) targetValue = String(id);
      }
    }
    if (targetValue) {
      const existe = Array.from(sel.options).some((opt) => opt.value === targetValue);
      if (!existe) {
        const opt = document.createElement("option");
        opt.value = targetValue;
        opt.textContent = selectedLabel || `Campanha #${targetValue}`;
    ...
    console.error("Erro ao carregar campanhas para edição", err);
    sel.innerHTML = '<option value="">Não vincular</option>';
    if (selectedValue) {
      const opt = document.createElement("option");
