document.addEventListener("DOMContentLoaded", function () {
    ...
    telInput.addEventListener("input", (e) => {
      ...
      e.target.setCustomValidity(
        validaTelefone(e.target.value)
          ? ""
          : "Telefone deve ter DDD e 8-9 dígitos"
      );
    });
  ...
  if (docInput) {
    ...
    docInput.addEventListener("blur", (e) => {
      e.target.setCustomValidity(
        validaCpfCnpj(e.target.value) ? "" : "Documento inválido"
      );
    });
  }
  ...
  if (emailInput) {
    emailInput.addEventListener("blur", (e) => {
      ...
      e.target.setCustomValidity(
        val && emailRegex.test(val) ? "" : "E-mail inválido"
      );
    });
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (emailInput)
        checks.push(emailRegex.test((emailInput.value || "").trim()));
